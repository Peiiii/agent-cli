...
async function main() {
    ...
    for (const plugin of pluginLoader.list()) {
        ...
        for (const [cmdName, cmd] of Object.entries(plugin.commands)) {
            const command = program.command(cmdName).description(cmd.description);
            // 允许插件设置自己的命令参数
            const handler = cmd;
            if (handler.setup) {
    ...
    program
        .command("plugin")
        .description("插件管理命令")
        .option("-i, --install <name>", "安装插件，例如: agent plugin -i git-assistant")
        .option("-r, --remove <name>", "移除插件，例如: agent plugin -r git-assistant")
        .option("-l, --list", "列出所有已安装的插件")
        .option("-d, --load <path>", "从本地路径加载插件，例如: agent plugin -d ./my-plugin")
        .option("--dev <path>", "开发模式：监视插件变化并自动重新加载")
        .addHelpText("after", `
示例:
